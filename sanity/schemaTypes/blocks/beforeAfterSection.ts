...
export const beforeAfterSection = defineType({
  ...
  fields: [
    ...
    defineField({ name: 'beforeLabel_en', title: 'Before Label (English)', type: 'string', initialValue: 'Before', validation: (Rule) => Rule.max(30) }),
    defineField({ name: 'beforeLabel_ar', title: 'تسمية قبل (Arabic)', type: 'string', validation: (Rule) => Rule.max(30) }),
