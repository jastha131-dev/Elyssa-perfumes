...
export const newsletterSection = defineType({
  ...
  fields: [
    ...
    defineField({ name: 'subtext_ar', title: 'النص التوضيحي (Arabic)', type: 'text', rows: 2 }),
    defineField({ name: 'buttonLabel_en', title: 'Button Label (English)', type: 'string', initialValue: 'Subscribe' }),
    defineField({ name: 'buttonLabel_ar', title: 'نص الزر (Arabic)', type: 'string' }),
    defineField({
      name: 'bgImage',
