      ...
      of: [
        defineArrayMember({
          ...
          fields: [
            ...
            defineField({ name: 'headline_en', title: 'Headline (English)', type: 'string', validation: (Rule) => Rule.max(80) }),
            defineField({ name: 'headline_ar', title: 'العنوان (Arabic)', type: 'string', validation: (Rule) => Rule.max(80) }),
            defineField({ name: 'body_en', title: 'Body (English)', type: 'text', rows: 3, validation: (Rule) => Rule.max(300) }),
            defineField({ name: 'body_ar', title: 'النص (Arabic)', type: 'text', rows: 3, validation: (Rule) => Rule.max(300) }),
