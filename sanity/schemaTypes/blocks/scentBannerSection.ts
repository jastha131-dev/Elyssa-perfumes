...
export const scentBannerSection = defineType({
  ...
  fields: [
    ...
    defineField({ name: 'eyebrow_en', title: 'Eyebrow Label (English)', type: 'string', description: 'e.g. "Explore by Note"' }),
    defineField({ name: 'eyebrow_ar', title: 'النص العلوي (Arabic)', type: 'string' }),
    ...
    defineField({ name: 'highlightWord_en', title: 'Highlighted Word(s) (English)', type: 'string', description: 'Italic gold portion of headline, e.g. "A Story"' }),
    defineField({ name: 'highlightWord_ar', title: 'الكلمة المميزة (Arabic)', type: 'string' }),
    defineField({ name: 'subtext_en', title: 'Body Text (English)', type: 'text', rows: 3 }),
    defineField({ name: 'subtext_ar', title: 'النص التوضيحي (Arabic)', type: 'text', rows: 3 }),
    defineField({
      name: 'bgImage',
