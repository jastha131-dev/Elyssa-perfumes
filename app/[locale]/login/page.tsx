    ...
    <div className="min-h-screen bg-stone-50 flex items-center justify-center px-4 pt-20 pb-12">
      ...
      >
        ...
        <div className="mb-8 text-center">
          <p className="mb-2 font-body text-xs uppercase tracking-[0.3em] text-camel-500">Welcome back</p>
          <h1 className="font-display text-3xl font-light text-ink-900">Sign In</h1>
        ...
        <div className="bg-white px-8 py-10 shadow-sm border border-stone-100">
          {error && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              className="mb-5 bg-red-50 border border-red-200 px-4 py-3 font-body text-sm text-red-600"
            >
              {error}
            </motion.div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <div className="mt-2 text-right">
                <Link href={`/${locale}/forgot-password`} className="font-body text-xs text-camel-500 hover:text-camel-700 transition-colors">
                  Forgot password?
                </Link>
              </div>
            </div>
            ...
            <button
              ...
              disabled={loading}
              className={cn(
                'w-full bg-camel-500 py-3.5 font-body text-sm font-semibold uppercase tracking-[0.18em] text-white transition-colors hover:bg-camel-600 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2'
              )}
            >
              {loading ? <><Loader2 size={14} className="animate-spin" /> Signing in&hellip;</> : 'Sign In'}
            </button>
          </form>

          <p className="mt-6 text-center font-body text-sm text-ink-400">
            No account?{' '}
            <Link href={`/${locale}/register`} className="text-camel-500 hover:text-camel-700 font-medium transition-colors">
              Create one
            </Link>
          </p>
        </div>
