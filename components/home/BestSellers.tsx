...
function BestSellerCard({ product, index }: BestSellerCardProps) {
  ...
  const fallbackUrl = LOCAL_PRODUCT_IMAGES[index % LOCAL_PRODUCT_IMAGES.length]
  const currentUrl = images[imgIdx]?.url || fallbackUrl
  const currentAlt = images[imgIdx]?.alt || ''
  const volumes = product.volume ?? []
  const selectedVol = volumes[selectedVolIdx]
  const displayPrice = selectedVol?.price ?? product.volume?.[0]?.price ?? product.price
  ...
  const handleAddToCart = useCallback(
    (e: React.MouseEvent) => {
      e.preventDefault()
      e.stopPropagation()
      const vol = selectedVol ?? product.volume?.[0] ?? { ml: 100, price: product.price }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Status badge — image overlay, position from Site Settings CSS vars */}
        {badgeLabel && (
          <div
        ...
        {images.length > 1 && (
          <>
            <button type="button" onClick={() => setImgIdx((i) => (i - 1 + images.length) % images.length)} className="absolute left-2 top-1/2 -translate-y-1/2 z-10 w-7 h-7 flex items-center justify-center bg-white/90 text-charcoal-800 rounded-full shadow-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 hover:bg-white" aria-label="Previous">
              <ChevronLeft size={13} strokeWidth={2.5} />
            </button>
            <button type="button" onClick={() => setImgIdx((i) => (i + 1) % images.length)} className="absolute right-2 top-1/2 -translate-y-1/2 z-10 w-7 h-7 flex items-center justify-center bg-white/90 text-charcoal-800 rounded-full shadow-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 hover:bg-white" aria-label="Next">
              <ChevronRight size={13} strokeWidth={2.5} />
            </button>
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-10 flex gap-1.5">
              {images.map((_, i) => (
                <button key={i} type="button" onClick={() => setImgIdx(i)} className={cn('h-1 rounded-full transition-all duration-200', i === imgIdx ? 'bg-white w-4 shadow-sm' : 'bg-white/45 w-1 hover:bg-white/70')} aria-label={`Image ${i + 1}`} />
              ))}
            </div>
          </>
        )}
      ...
      >
        ...
        <div className="flex items-start gap-1.5 mt-0.5" style={{ justifyContent: 'var(--card-price-justify, flex-start)' }}>
          <Link href={`/${locale}/products/${product.slug}`} className="flex-1 font-body text-charcoal-900 leading-snug hover:text-camel-600 transition-colors" style={{ fontSize: 'var(--card-name-size, 16px)' }}>
            {productName}
          </Link>
          <button
            type="button"
            onClick={handleWishlist}
            className={cn('flex-shrink-0 mt-0.5 transition-colors', isWishlisted ? 'text-rose-400' : 'text-rose-300 hover:text-rose-400')}
        ...
        <div className="flex items-center gap-1.5" style={{ justifyContent: 'var(--card-price-justify, flex-start)' }}>
          <div className="flex items-center gap-0.5">
            {Array.from({ length: 5 }).map((_, i) => (
              <Star key={i} size={11} className={i < Math.round(avgRating) ? 'text-yellow-400' : 'text-charcoal-200'} fill={i < Math.round(avgRating) ? 'currentColor' : 'none'} strokeWidth={i < Math.round(avgRating) ? 0 : 1.5} />
            ))}
          </div>
          {reviewCount > 0 && (
            <span className="font-body text-[9px] font-semibold uppercase tracking-[0.12em] text-charcoal-400">{reviewCount}</span>
          )}
        </div>
            ...
            {volumes.map((vol, i) => (
              <button
                ...
                className={cn(
                  ...
                  i === selectedVolIdx ? 'border-charcoal-800 text-charcoal-900 bg-white' : 'border-charcoal-200 text-charcoal-400 bg-white hover:border-charcoal-500'
                )}
              >
                {vol.ml}ml
              </button>
        ...
        <div className="flex items-baseline gap-2 mt-0.5" style={{ justifyContent: 'var(--card-price-justify, flex-start)' }}>
          <span className="font-body font-semibold text-charcoal-900" style={{ fontSize: 'var(--card-price-size, 14px)' }}>
            {formatPrice(displayPrice)}
          </span>
          {product.compareAtPrice && product.compareAtPrice > product.price && (
            <span className="text-xs text-charcoal-400 line-through">{formatPrice(product.compareAtPrice)}</span>
          )}
        </div>
        ...
        >
          {addedToCart ? (isAr ? 'تمت الإضافة ✓' : 'Added ✓') : `${formatPrice(displayPrice)} | ${isAr ? 'أضف للسلة' : 'Add to Cart'}`}
        </motion.button>
      </div>
    </motion.div>
  )
